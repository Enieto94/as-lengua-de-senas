<template>
  ...
  <main>
    ...
    <div class="letters-c">
      <div :class="
      presentacion.visto == false ? 'letter-item' :
      presentacion.visto == true ? 'letter-item active':
      'letter-item'"   v-for="presentacion in presentaciones" :key="presentacion.id" @click="mostrarImg(presentacion.url_src,presentacion.id,presentacion.visto)" >
        <!-- <img :src="presentacion.url_src" alt=""> -->
        
          <span>{{ presentacion.nom_letra }}</span>

          <div v-if="presentacion.visto">
...
export default {
  ...
  methods: {
    mostrarImg(url_src, id, visto) {
      ...
      fetch('http://127.0.0.1:8000/api/updatepresentacion/'+id, {
        method: 'PUT',
        headers: {
