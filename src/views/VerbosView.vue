<template>
  ...
  <main>
    ...
    <div class="letters-c">
      <div :class="
      verbo.visto == false ? 'letter-item' :
      verbo.visto == true ? 'letter-item active':
      'letter-item'"   v-for="verbo in verbos" :key="verbo.id" @click="mostrarImg(verbo.url_src,verbo.id,verbo.visto)" >
        <!-- <img :src="verbo.url_src" alt=""> -->
        
          <span>{{ verbo.nom_letra }}</span>

          <div v-if="verbo.visto">
...
export default {
  ...
  methods: {
    mostrarImg(url_src, id, visto) {
      ...
      fetch('http://127.0.0.1:8000/api/updateverbo/'+id, {
        method: 'PUT',
        headers: {
